# Blind 75 Progress Tracker

## Overall Progress
- **Total Problems:** 75
- **Completed:** 7
- **In Progress:** 0
- **Generated:** 8 (All Arrays & Hashing problems ready)
- **Remaining:** 67
- **Completion Percentage:** 1%

---

## Arrays & Hashing (8 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Two Sum | ✅ | 2026-01-16 |
| Contains Duplicate | ✅ | 2026-01-17 | 📝 |
| Valid Anagram | ✅ | 2026-01-17 | 📝 |
| Group Anagrams | ✅ | 2026-01-17 | 📝 |
| Top K Frequent Elements | ✅ | 2026-01-17 | 📝 |
| Product of Array Except Self | ✅ | 2026-01-18 | 📝 |
| Valid Sudoku | ⬜ | - | 📝 |
| Longest Consecutive Sequence | ✅ | 2026-01-17 | 📝 |
| **Progress:** 7/8 | | |
| **Status:** All problems generated with test cases | |

---

## Two Pointers (5 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Valid Palindrome | ⬜ | - |
| Two Sum II - Input Array is Sorted | ⬜ | - |
| 3Sum | ⬜ | - |
| Container With Most Water | ⬜ | - |
| Trapping Rain Water | ⬜ | - |
| **Progress:** 0/5 | | |

---

## Sliding Window (4 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Best Time to Buy and Sell Stock | ⬜ | - |
| Longest Substring Without Repeating Characters | ⬜ | - |
| Longest Repeating Character Replacement | ⬜ | - |
| Permutation in String | ⬜ | - |
| **Progress:** 0/4 | | |

---

## Binary Search (4 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Search in Rotated Sorted Array | ⬜ | - |
| Find Minimum in Rotated Sorted Array | ⬜ | - |
| Search in Rotated Sorted Array II | ⬜ | - |
| Median of Two Sorted Arrays | ⬜ | - |
| **Progress:** 0/4 | | |

---

## Linked List (6 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Reverse Linked List | ⬜ | - |
| Merge Two Sorted Lists | ⬜ | - |
| Linked List Cycle | ⬜ | - |
| Reorder List | ⬜ | - |
| Remove Nth Node From End of List | ⬜ | - |
| Add Two Numbers | ⬜ | - |
| **Progress:** 0/6 | | |

---

## Trees (10 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Invert Binary Tree | ⬜ | - |
| Maximum Depth of Binary Tree | ⬜ | - |
| Same Tree | ⬜ | - |
| Subtree of Another Tree | ⬜ | - |
| Lowest Common Ancestor of a Binary Tree | ⬜ | - |
| Binary Tree Level Order Traversal | ⬜ | - |
| Validate Binary Search Tree | ⬜ | - |
| Kth Smallest Element in a BST | ⬜ | - |
| Construct Binary Tree from Preorder and Inorder Traversal | ⬜ | - |
| Serialize and Deserialize Binary Tree | ⬜ | - |
| **Progress:** 0/10 | | |

---

## Graphs (9 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Number of Islands | ⬜ | - |
| Clone Graph | ⬜ | - |
| Pacific Atlantic Water Flow | ⬜ | - |
| Surrounded Regions | ⬜ | - |
| Rotting Oranges | ⬜ | - |
| Course Schedule | ⬜ | - |
| Course Schedule II | ⬜ | - |
| Redundant Connection | ⬜ | - |
| Word Ladder | ⬜ | - |
| **Progress:** 0/9 | | |

---

## 1D Dynamic Programming (10 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Climbing Stairs | ⬜ | - |
| Min Cost Climbing Stairs | ⬜ | - |
| House Robber | ⬜ | - |
| House Robber II | ⬜ | - |
| Longest Palindromic Substring | ⬜ | - |
| Palindromic Substrings | ⬜ | - |
| Decode Ways | ⬜ | - |
| Coin Change | ⬜ | - |
| Maximum Product Subarray | ⬜ | - |
| Word Break | ⬜ | - |
| **Progress:** 0/10 | | |

---

## Intervals (5 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Non-overlapping Intervals | ⬜ | - |
| Meeting Rooms | ⬜ | - |
| Meeting Rooms II | ⬜ | - |
| Merge Intervals | ⬜ | - |
| Insert Interval | ⬜ | - |
| **Progress:** 0/5 | | |

---

## Greedy (7 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Maximum Subarray | ⬜ | - |
| Jump Game | ⬜ | - |
| Jump Game II | ⬜ | - |
| Gas Station | ⬜ | - |
| Hand of Straights | ⬜ | - |
| Merge Triplets to Form Target Triplet | ⬜ | - |
| Partition Labels | ⬜ | - |
| **Progress:** 0/7 | | |

---

## Backtracking (8 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Subsets | ⬜ | - |
| Subsets II | ⬜ | - |
| Permutations | ⬜ | - |
| Permutations II | ⬜ | - |
| Combination Sum | ⬜ | - |
| Combination Sum II | ⬜ | - |
| Word Search | ⬜ | - |
| Palindrome Partitioning | ⬜ | - |
| **Progress:** 0/8 | | |

---

## Bit Manipulation (6 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Single Number | ⬜ | - |
| Sum of Two Integers | ⬜ | - |
| Number of 1 Bits | ⬜ | - |
| Counting Bits | ⬜ | - |
| Missing Number | ⬜ | - |
| Reverse Bits | ⬜ | - |
| **Progress:** 0/6 | | |

---

## Heap / Priority Queue (3 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Kth Largest Element in an Array | ⬜ | - |
| Find Median from Data Stream | ⬜ | - |
| Task Scheduler | ⬜ | - |
| **Progress:** 0/3 | | |

---

## Advanced Graphs (3 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Kruskal's Algorithm - MST | ⬜ | - |
| Prim's Algorithm - MST | ⬜ | - |
| Dijkstra's Algorithm - Shortest Path | ⬜ | - |
| **Progress:** 0/3 | | |

---

## 2D Dynamic Programming (7 problems)
| Problem | Status | Date Completed |
|---------|--------|----------------|
| Unique Paths | ⬜ | - |
| Unique Paths II | ⬜ | - |
| Longest Common Subsequence | ⬜ | - |
| Best Time to Buy and Sell Stock with Cooldown | ⬜ | - |
| Coin Change II | ⬜ | - |
| Target Sum | ⬜ | - |
| Interleaving String | ⬜ | - |
| **Progress:** 0/7 | | |

---

## Legend
- ⬜ = Not Started
- 🟡 = In Progress
- ✅ = Completed
- 🔄 = Reviewing

## Notes
- Keep this file updated as you complete each problem
- Consider revisiting completed problems after a few days for reinforcement
- Track any difficulties or insights you discover
- Time yourself on each attempt to improve speed